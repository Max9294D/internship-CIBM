...
def trainIters(encoder, decoder, train_data_loader, n_epochs, logger, save_dir='', st_epoch=-1, val_data_loader=None,
    ...
    if st_epoch > 0:
        enc_ckp_file = os.path.join(checkpoint_dir, f"ckpt_ep-{st_epoch}_encoder2.pt")
        dec_ckp_file = os.path.join(checkpoint_dir, f"ckpt_ep-{st_epoch}_decoder2.pt")
    ...
    for epoch in range(n_epochs):
        ...
        if save_dir and ((epoch+1) % check_freq == 0 or epoch==n_epochs):
            checkpoint_encoder = os.path.join(checkpoint_dir, f"ckpt_ep-{st_epoch+epoch}_encoder2.pt")
            ...
            #           
            checkpoint_decoder = os.path.join(checkpoint_dir, f"ckpt_ep-{st_epoch+epoch}_decoder2.pt")
            torch.save({'epoch': epoch + st_epoch,
    ...
    #save model
    torch.save(encoder.state_dict(), os.path.join(save_dir, 'lstm_encoder2.pt'))
    torch.save(decoder.state_dict(), os.path.join(save_dir, 'lstm_decoder2.pt'))
    ...
    if len(plot_losses)>1:
        ...
        if os.path.isdir(checkpoint_dir):
            np.save(os.path.join(checkpoint_dir, 'training_avg_losses2.npy'), np.array(plot_losses))
            if len(plot_val_losses)>1: 
                np.save(os.path.join(checkpoint_dir, 'val_avg_losses2.npy'), np.array(plot_val_losses))
